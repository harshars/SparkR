...
if (nchar(Sys.getenv("SPARK_HOME")) < 1) {
  Sys.setenv(SPARK_HOME = "C://Apache/spark-2.3.1-bin-hadoop2.7/")
}
...
auction_data = read.df('C:/Users/rshars/Desktop/Shared drive/Spark R/Week 1/auction.csv','csv',inferSchema = 'True', header = 'True')
class(auction_data)
...
distinct(auction_data$auctionid)
...
bidder= summarize(groupBy(auction_data, auction_data$bidder,auction_data$item), sum = sum (auction_data$bid))
...
##Doubt : Does it work similar to aggregate or different?
#column bidder and max price
result <- gapply(
  auction_data,
  "bidder",
  function(key, x) {
    y <- data.frame(key, max(x$price))
    colnames(y) <- c("bidder", "max_price")
    y
  })
...
#Can we make a list? How to get count of list?
#Taking unique of a column
#Taking mode of a column without sql
